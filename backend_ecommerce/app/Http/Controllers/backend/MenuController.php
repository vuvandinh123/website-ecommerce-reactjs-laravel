...
class MenuController extends Controller
{
    ...
    {
        $menus = Menu::where([['status', '=', 1], ['table_name', '=', "category"]])->orderBy('sort_order', 'asc')->get();
        $cateId = [];
    ...
    {
        $menus = Menu::where([['status', '=', 1], ['table_name', '=', "brand"]])->get();
        $brandId = [];
        foreach ($menus as $key => $value) {
    ...
    private function create($data,$table)
    {
        ...
        $menu->name = $data->name;
        $menu->type = "mainmenu";
        $menu->link = "/" . "categories/" . $data->slug;
        $menu->sort_order =  1;
        $menu->parent_id = 0;
